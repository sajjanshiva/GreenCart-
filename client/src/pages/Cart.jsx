...
const Cart = () => {
    ...
    const { products,currency,cartItems, getCartCount,getCartAmount,addToCart ,
        removeCartItem,updateCartItem,navigate,axios,user, setCartItems } = useAppContext();
    ...
    // Convert cartItems object to an array of products
    //key is product id and value is quantity
    const getCart =() =>{
        let tempArray = [];
        ...
        try{
            ...
            if(data.success){
                ...
                if(data.addresses.length > 0){
                    ...
                    toast.error("No address found, please add one");
                }
            }
        }catch(error){
        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                ...
                <button onClick={() => navigate("/products")} className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
                    <img src={assets.arrow_right_icon_colored} alt="arrow_right_icon_colored" className="w-4 h-4"/>
                    Continue Shopping
                </button>
                            ...
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                { addresses.map((address, index) => (
                                  ...
                                  onClick={() => {setSelectedAddress(address); setShowAddress(false)}} className="text-gray-500 p-2 hover:bg-gray-100">
                                    {address.state}, {address.city}, {address.street}, {address.country}
                                </p>
                                )) 
                                }
